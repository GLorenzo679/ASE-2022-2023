...
void enable_timer( uint8_t timer_num )
{
  ...
  else if ( timer_num == 1 )
  {
	...
	else if ( timer_num == 2 ) {		//enable timer 2
		LPC_TIM2->TCR = 1;
  }
  return;
}
...
void disable_timer( uint8_t timer_num )
{
  ...
  else if ( timer_num == 1 )
  {
	...
	else if ( timer_num == 2 ) {		//disable timer 2
		LPC_TIM2->TCR = 0;
  }
  return;
}
...
uint32_t init_timer( uint8_t timer_num, uint32_t TimerInterval_1, uint32_t TimerInterval_2)
{
  if ( timer_num == 0 )
  {
	LPC_TIM0->MR0 = TimerInterval_1;
...
//  LPC_TIM0->PR = 20;
  ...
  else if ( timer_num == 1 )
  {
	...
	return (1);
		...
		LPC_TIM2->MCR = 25;								//enable interrupt for MR0 and MR1
